...
var NewBlockE409 = /*#__PURE__*/function () {
   ...
   * @param {(module:model/AlreadyExists|module:model/InvalidData)} instance The actual instance to initialize NewBlockE409.
   */
  function NewBlockE409() {
    ...
    if (match > 1) {
      throw new Error("Multiple matches found constructing `NewBlockE409` with oneOf schemas AlreadyExists, InvalidData. Input: " + JSON.stringify(instance));
    } else if (match === 0) {
      this.actualInstance = null; // clear the actual instance in case there are multiple matches
      throw new Error("No match found constructing `NewBlockE409` with oneOf schemas AlreadyExists, InvalidData. Details: " + errorMessages.join(", "));
    } else {// only 1 match
      // the input is valid
    }
  }
  ...
  _createClass(NewBlockE409, [{
    ...
    value: function setActualInstance(obj) {
      this.actualInstance = NewBlockE409.constructFromObject(obj).getActualInstance();
    }
 ...
 * @member {String} code
 */
_defineProperty(NewBlockE409, "fromJSON", function (json_string) {
  return NewBlockE409.constructFromObject(JSON.parse(json_string));
});
...
NewBlockE409.OneOf = ["AlreadyExists", "InvalidData"];
